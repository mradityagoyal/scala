...
trait GameDef {
  ...
  case class Block(b1: Pos, b2: Pos) {
    ...
    // checks that b1 and b2 are next to each other or on top of each other. 
    require(Math.abs(b1.x - b2.x) <= 1 && Math.abs(b1.y - b2.y) <= 1, "Invalid block position b1=" + b1+ ", b2 = " + b2)
